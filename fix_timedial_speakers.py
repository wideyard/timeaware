...
BASE = Path('data-converted/TimeDial/full_T1')
FILES = [
    BASE / 'TimeDial_single.jsonl',
    BASE / 'TimeDial_multi_v1.jsonl',
    BASE / 'TimeDial_multi_v2.jsonl',
    BASE / 'TimeDial_multi_v3.jsonl',
]

DIALOGUE_LINE = re.compile(r'^\s*([AB])\s*:\s*(.*)$')
...
def split_dialogue_lines(block: str):
    ...
    for raw in block.splitlines():
        ...
        if m:
            role = 'user' if m.group(1) == 'A' else 'assistant'
...
def split_single_user_message(content: str):
    ...
    out.append({'role': 'user', 'content': 'Question:' + qtail})
    return out
...
def transform_obj(obj):
    ...
    for idx, m in enumerate(msgs):
        ...
        if role != 'user' or 'A:' not in content or 'B:' not in content:
            new_msgs.append(m)
            continue
